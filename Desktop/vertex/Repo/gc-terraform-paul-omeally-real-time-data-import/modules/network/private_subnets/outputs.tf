output "az_subnet_ids" {
  value       = zipmap(slice(data.aws_availability_zones.availability_zones.names,0,2), aws_subnet.private.*.id)
  description = "Map of AZ names to subnet IDs"
}

output "az_route_table_ids" {
  value       = zipmap(slice(data.aws_availability_zones.availability_zones.names,0,2), aws_route_table.private.*.id)
...
# also as per description is for public, not private, subnets.
# output "az_ngw_ids" {
...
output "az_subnet_arns" {
  value       = zipmap(slice(data.aws_availability_zones.availability_zones.names,0,2), aws_subnet.private.*.arn)
